...
import { Library, LibraryData, Version, VersionData } from '../models/library.model';
...
export class LibraryService {
  ...
  private getApiVersion(): Observable<string> {
    return this._http.get<VersionData>(`${this._apiUrl}/version?api_key=${this._apiKey}`)
      .pipe(
        catchError((err) => {
  ...
  private getLibraries(param: string, version: string): Observable<Library[]> {
    return this._http.get<LibraryData[]>(`${this._apiUrl}/${version}/datasets/526/rows?${param}api_key=${this._apiKey}`)
      .pipe(
        catchError((err) => {
